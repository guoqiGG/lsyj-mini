	...
	<view class="submit-order">
		...
		<view class="submit-order-index">
			...
			<view class="submit-order-coupon">
				<view class="submit-order-coupon-select">
					<view class="submit-order-coupon-select-left">
						平台优惠券：
					</view>
					<view class="submit-order-coupon-select-button">
						<text v-if="totalCouponAmount<=0" style="font-size: 24rpx;color: #9E9E9E;" @click="selectCouponClick()">选择优惠券></text>
						<text style="color: #C53032;" v-else @click="selectCouponClick()">￥-{{ parsePrice(totalCouponAmount)[0] }}.{{ parsePrice(totalCouponAmount)[1] }}</text>
							...
							<view v-for="(item,index) in couponList" class="popup-style-content-wrap" :key="item.id">
								...
								<view class="popup-style-content-wrap-right">
									...
									<view class="popup-style-content-wrap-right-button">
										<u-checkbox   :disabled="item.disabled"   :key="item.id" 
										                :name="item.id"  @change="getCheckClick(item.id)">
										</u-checkbox>
	...
	export default {
		...
		onLoad(options) {
			if (uni.getStorageSync('bbcOrderItem')) {
				...
				this.couponList=this.orderItemInfo.unValidCouponList
