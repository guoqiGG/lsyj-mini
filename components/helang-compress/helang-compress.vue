<template>
  <view class="compress">
    <canvas :style="{ width: canvasSize.width, height: canvasSize.height }" canvas-id="myCanvas" />
  </view>
</template>
<script>
export default {
  ...
  methods: {
    // 压缩
    compress(params, tempFile, maxSizeMB) {
                ...
                success: (res) => {
                  ...
                  uni.getFileInfo({
                    ...
                    success: (res2) => {
                      ...
                      resolve(this.compress(params, res2, maxSizeMB))
                    }
                  })
                },
                fail: (err) => {
