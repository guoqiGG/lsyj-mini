    ...
    <view v-if="pageType" class="sku" @tap="showSku">
      ...
      <view class="sku-con">
        <block v-for="(skuItem, index) in selectedPropShowList" :key="index">
          <text decode="true">{{
            index < selectedPropShowList.length - 1
              ? skuItem + "，"
              : skuItem + "&nbsp; &nbsp;"
          }}</text>
        </block>
      ...
      <view class="pup-sku-main" @tap.stop>
        <view class="pup-sku-header">
          ...
          <view class="pup-sku-prod">
            ...
            <view
              v-if="
                findSku && prodType === 2 && activityInfo.activityStatus === 2
              "
              class="pup-sku-price"
              >￥
            ...
            <view
              v-if="
                (findSku && skuShowType === 0 && prodType === 0) ||
                prodType === 3 ||
                (prodType === 1 && skuShowType === 0) ||
                (prodType === 2 && activityInfo.activityStatus === 1) ||
                activityInfo.activityStatus === undefined
              "
              class="pup-sku-price"
            >
              <block
                v-if="
                  prodType !== 3 || defaultSku.matchingPrice || defaultSku.price
                "
              >
                <text
                  ...
                  >￥{{
                    parsePrice(defaultSku.matchingPrice || defaultSku.price)[0]
                  }}</text
                >
              ...
              <text
                v-if="
                  (defaultSku.matchingPrice || defaultSku.price) &&
                  defaultSku.skuScore
                "
                >+</text
              >
              <text
                ...
                class="pup-sku-price-int"
                >{{ defaultSku.skuScore }}{{ i18n.integral }}</text
              >
              <text
                v-if="
                  (defaultSku.price || defaultSku.skuScore) &&
                  defaultSku.skuVO.number > 0
                "
                >+</text
              >
              <text class="pup-sku-price-int" v-if="defaultSku.skuVO.number > 0"
                >{{ defaultSku.skuVO.number }}礼品券
              </text>
            </view>
            <!-- 拼团商品价格 -->
            <view
              ...
              >￥
              <text class="pup-sku-price-int">{{
                parsePrice(defaultGroupSku.actPrice || defaultSku.actPrice)[0]
              }}</text>
              .{{
            ...
            <view class="pup-sku-prop">
              ...
              <view v-if="findSku" style="margin-top: 16rpx">
                ...
                <text decode="true">{{
                  "&nbsp;" +
                  (prodType === 2 &&
                  (defaultSku.seckillStocks === 0 || defaultSku.seckillStocks)
                    ? defaultSku.seckillStocks
                    : defaultSku.stocks)
                }}</text>
                <text
                  v-if="
                    (prodType === 2 || prodType === 1) &&
                    activityInfo.maxNum >= 1 &&
                    skuShowType === 1
                  "
                  style="margin-left: 20rpx"
                  >{{ i18n.purchaseLimit }}</text
                >
                <text
                  v-if="
                    (prodType === 2 || prodType === 1) &&
        ...
        <view class="pup-sku-body">
          ...
          <view
            v-if="
              mold === 1 &&
              (virtualInfo.writeOffNum !== 0 ||
                (virtualInfo.writeOffNum === 0 && virtualInfo.isRefund === 0))
            "
          ...
          >
            ...
            <block v-if="virtualInfo.writeOffNum !== 0">
              ...
              <text v-if="virtualInfo.writeOffTime === -1">{{
                i18n.longTermValidity
              }}</text>
              <text v-else-if="virtualInfo.writeOffTime === 0"
                >{{ i18n.afterPurchase }} {{ virtualInfo.writeOffStart }}
                {{ i18n.to }} {{ virtualInfo.writeOffEnd }}
                <text v-if="!isEn">{{ i18n.effective }}</text></text
              >
              <text v-else-if="virtualInfo.writeOffTime === 1">{{
                i18n.validOnTheSameDay
              }}</text>
              <text v-else
                >{{ i18n.purchase }}{{ virtualInfo.writeOffTime
                }}{{ i18n.validDay }}</text
              >
            </block>
            <!-- isRefund 0不支持退款 1支持退款 -->
            <text v-if="virtualInfo.isRefund === 0"
              ><text v-if="virtualInfo.writeOffNum !== 0">，</text
                ...
                >
                  ...
                  <view class="con">
                    <text
                      ...
                      :class="[
                        'item-item',
                        selectedPropList.indexOf(key + ':' + skuLineItem) !== -1
                          ? 'active'
                          : '',
        ...
        <view
          ...
          :class="
            ...
            (isJionGroup
              ? ''
              : findSku && !prodNumBiggerThanStock()
              ? ''
              : 'gray')
          "
        >
          ...
          <view
            v-if="
              preSellStatus == 1 &&
              !(prodType === 2 && activityInfo.activityStatus === 2)
            "
          ...
          <view
            ...
            :class="[
              'btn',
              activityInfo.activityStatus === 1 ? 'gray-btn' : 'buy',
            ]"
            @tap="handleBuyNow(1)"
            >{{ i18n.startAGroup }}
          </view>
        ...
        <view
          ...
          :class="
            'pup-sku-footer ' +
            (findSku && !prodNumBiggerThanStock() ? '' : 'gray')
          "
        >
...
export default {
  ...
  methods: {
    ...
    groupSku() {
      const skuList =
        this.activityInfo.activityStatus == 2 && this.activityInfo.seckillId
          ? this.activityInfo.seckillSkuList
          : this.skuList;
      ...
      for (var i = 0; i < skuList.length; i++) {
        ...
        for (var j = 0; j < propList.length; j++) {
          ...
          var propval = [
            propList[j].slice(0, index),
            propList[j].substring(index + 1),
          ]; // ["版本","公开版"]
    ...
    setSkuImg() {
      for (let i = 0; i < this.skuList.length; i++) {
        ...
        if (
          skuObj.substring(index + 1) ===
          this.selectedPropObjList[0][skuObj.slice(0, index)]
        ) {
    ...
    toChooseItem(skuGroupItemIndex, skuLineItem, key) {
      this.selectedPropObjList[skuGroupItemIndex] =
        this.selectedPropObjList[skuGroupItemIndex] || {};
    ...
    setDefaultGroupSku() {
      const groupSkuList =
        this.activityInfo.groupSkuList || this.activityInfo.seckillSkuList;
      if (groupSkuList) {
    ...
    onCountPlus: function () {
      ...
      // 默认库存数量
      const defaultSkuStock = parseInt(
        this.prodType === 2 &&
          (this.defaultSku.seckillStocks === 0 || this.defaultSku.seckillStocks)
          ? this.defaultSku.seckillStocks
          : this.defaultSku.stocks
      );
      // 是否限购
      const isRestriction =
        this.activityInfo.maxNum &&
        this.activityInfo.maxNum !== -1 &&
        this.skuShowType === 1 &&
        this.activityInfo.maxNum <= defaultSkuStock;
      // limit 最大可购买数量: 活动商品不限购则为当前sku库存；活动商品限购时，当库存大于限购数量则为限购数量，否则为库存数量
      const limit = !isRestriction
        ? defaultSkuStock
        : defaultSkuStock > this.activityInfo.maxNum
        ? this.activityInfo.maxNum
        : defaultSkuStock;
      if (isRestriction) {
        if (prodNum >= limit) {
          uni.showToast({
            title:
              isRestriction && limit
                ? `${this.i18n.purchaseLimit} ${this.activityInfo.maxNum} ${this.i18n.piece}`
                : this.i18n.insufficientStock,
            icon: "none",
          });
        }
    ...
    prodNumBiggerThanStock() {
      ...
      const defaultSkuStock =
        this.pageType == 2
          ? this.defaultSku.seckillStocks
          : this.defaultSku.stocks;
      return this.defaultSku.stocks === 0
        ? true
        : defaultSkuStock === 0
        ? true
        : this.prodNumSelf > defaultSkuStock;
    },
