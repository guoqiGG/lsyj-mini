...
<template>
  <view :class="'Mall4j coupon-item ' + (canUse ? '' : 'gray')">
    <view class="left">
      <view v-if="item.couponType == 1 || item.couponType == 3" class="num">￥<text class="coupon-price">
    ...
    <view class="right">
      <view class="c-des">
        ...
        <view style="font-size: 24rpx;color:#aaaaaa;margin-top:10rpx;font-weight:300;">{{ item.suitableProdType == 0 ?
          i18n.allUniversal :
          item.suitableProdType == 3 ? '指定分类可用' + '(' + item.couponGroupName + '分类)' : i18n.specifiedItems }}</view>
      </view>
      <view v-if="item.shopId != 0 && item.shopName" class="c-desc">
        {{ i18n.purchaseLimit }} [{{ item.shopName }}] {{ item.suitableProdType == 0 ? i18n.allUniversal :
          i18n.specifiedItems
        }}
      </view>
      <view v-if="item && update" class="c-date">
        <view v-if="showTimeType == 1 && item.validTimeType == 2" class="c-data-info">{{ i18n.getCoupons + ' ' }}{{
          item.validDays }}{{ i18n.invalid }}</view>
